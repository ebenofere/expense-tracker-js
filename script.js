...
function addTransactionDOM(transaction) {
  ...
  item.classList.add(`${transaction < 0 ? "minus" : "plus"}`);

  item.innerHTML = `${transaction.text} <span>${sign}${Math.abs(transaction.amount)}</span><button class="delete-btn" onclick=${removeTransaction(transaction.id)}>x</button></li>`;
